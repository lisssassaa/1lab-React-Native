...
const ImageList: React.FC<ImageListProps> = ({
  ...
  const handleDelete = (imageId: string) => { //Подтверждение 
    Alert.alert(
      'Удалить изображение',
  ...
  const renderImageItem = ({ item }: { item: ImageData }) => ( //карточка изображения 
  ...
  const renderEmptyImage = () => ( // пустая галерея 
    <View style={styles.emptyImageContainer}>
      <Text style={styles.emptyImageText}>Нет изображений</Text>
      <Text style={styles.emptyImageSubtext}>
        Нажмите кнопку «Добавить изображение» для загрузки
      </Text>
    </View>
  );
